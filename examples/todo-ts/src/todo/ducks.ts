...
const actions = {
   ...
   **/
  initialise: function (text: string) {
    return {
      type: "INIT",
      text,
      completed: false,
   ...
   **/
  onSetCompleted: function (completed: boolean) {
    return {
      type: "COMPLETED",
      completed,
    };
 ...
 **/
function _reducer(state = { text: "", completed: false }, action) {
  switch (action.type) {
    case "INIT":
...
export const reducer = bindReducer(_reducer, { actions });
